...
function QuestionDetail() {
  ...
  */
  ...
  const handleRelancer = async () => {
    ...
    try {
      async function GetData() {
        var response = await axios.patch('http://localhost:8080/api/cyusers/nouv_demande_validation/'+idUser+'/'+idQuestion,
        {
          "question" : question,
  ...
  const handleSupprimer = async () => {
    ...
    try {
      async function GetData() {
        var response = await axios.patch('http://localhost:8080/api/cyusers/supprimer_question/'+idUser+'/'+idQuestion);
        setData(response.data)
      }
      GetData();
      alert('Question supprimmée');
      navigate('/Menu');
    }
    catch(error) {
      //try to fix the error or
    ...
    <div className="question-details-container">
      ...
      <div>
          ...
          <select className="select-question-details" name="filiere" defaultValue={filiere} onChange={(e) => setFiliere(e.target.value)}>
            ...
            {// afficher les options correctement
        ...
        <label>
          ...
          <div className="reponses-field-question-details">
            <textarea defaultValue={data.correction} onChange={(e) => setCorrection(e.target.value)} minLength="150" />
          </div>
        </label>
        <label>
          <div className="titre-question-details">
        ...
        <div className="buttons">
          <input className="refuser" type="button" value="Supprimer" onClick={handleSupprimer}/>
          <input className="approuver" type="button" value="Relancer" onClick={handleRelancer}/>
        </div>
      </form>
    </div>
  );

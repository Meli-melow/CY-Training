...
function QuestionDetail() {
  ...
  */
  ...
  useEffect(() => { // on met à jour correctement les states (ils ne sont pas définies sinon)
    setReponses(data.reponses);
  ...
  const handleApprove = () => {
    ...
    // Vérification qu'il y a une seule bonne réponse sélectionnée
    ...
    try {
        ...
        navigate("/questionapprouveesList");
      }
      catch(error) {
        console.log(error.message)
      }
  ...
  const handleRemettre = () => {
    ...
    try {
      async function GetData() {
        var response = await axios.patch('http://localhost:8080/api/questions/remise_attente/'+idQuestion);
        setData(response.data)
      }
      GetData();
      alert('Question remise en attente');
      navigate("/questionapprouveesList");
    }
    catch(error) {
      //try to fix the error or
    ...
    <div className="question-details-container">
      ...
      <div>
          ...
          <select className="select-question-details" name="filiere" defaultValue={filiere} onChange={(e) => setFiliere(e.target.value)}>
            ...
            {// les commentaires ici  et après sont une ébauches pour faire fonctionner les options correctement
            /*dataFiliere.map((f, index) => (
          ...
          {data.reponses && data.reponses.map((reponse, index) => (
            <div className="reponses-field-question-details">  
              ...
              <input type="radio" id={index} name={index} defaultChecked={!data.indBonneRep.includes(index)/* On regarde si la réponse est mauvaise*/} required/>
                ...
                {/*lien du check et du x : https://textkool.com/fr/symbols/tick-check-mark-symbols?utm_content=cmp-true*/}
              </div>
        ...
        <div className="buttons">
          <input className="remettre" type="button" value="Remettre en attente" onClick={handleRemettre}/>
          <input className="approuver" type="button" value="Corriger" onClick={handleApprove}/>
        </div>
      </form>
    </div>
  );
